...
-- join_history 테이블에 존재하는 부서 출력
SELECT a.department_id
,a.department_name
FROM departments a
WHERE EXISTS (SELECT * -- select 는 의미없음 where의 내용에 해당되는
                        -- 데이터가 존재하는지만 체크
                FROM job_history b
...
SELECT *
FROM member
WHERE  REGEXP_LIKE(mem_mail, '^[a-zA-Z]{3}@'); --소문자, 대문자 3글자 + @ 패턴 
SELECT *
FROM member
WHERE  REGEXP_LIKE(mem_mail, '^[a-zA-Z]{3,5}@'); --소문자, 대문자 3-5글자 + @ 패턴
-- mem_add2 문자 띄어쓰기 문자 패턴의 데이터를 출력하시오
SELECT mem_name
,mem_add2
FROM member
--WHERE REGEXP_LIKE(mem_add2,'. .'); --띄어쓰기 가 있는 주소
--WHERE REGEXP_LIKE(mem_add2,'[가-힝] [0-9]'); --한글
--WHERE REGEXP_LIKE(mem_add2,'[가-힝]$'); --한글로 끝나는 $ -> 끝나는패턴
--WHERE REGEXP_LIKE(mem_add2,'^[가-힝]*[가-힝]$'); 
...
SELECT REGEXP_SUBSTR('pang su hi 1234', '[0-9]')    -- default 1,1
        ...
        ,REGEXP_SUBSTR('pang su hi 1234', '[^0-9]+')
        FROM dual;
SELECT mem_add1 -- 띄어쓰기를 기준으로 2번째 출현하는 주소를 출력하시오
FROM  member;
